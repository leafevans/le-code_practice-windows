...
--- 查询所有学生的姓名、学号和所在系
SELECT Sname AS '姓名', Sno AS '学号', Sdept AS '所在系' FROM Student;
...
SELECT Sname AS '姓名', 2007 - Sage AS '出生年份' FROM Student;

-- 查询所有学生的姓名、固定标签 'Year of Birth: '、出生年份和小写系别
SELECT Sname AS '姓名', 'Year of Birth: ' AS '标签', 2007 - Sage AS '出生年份', LOWER(Sdept) AS '小写系别'
FROM Student;

-- 查询所有学生的姓名（别名为 NAME）、固定标签（别名为 BIRTH）、出生年份（别名为 BIRTHDAY）和小写系别（别名为 DEPARTMENT）
SELECT Sname AS 'NAME', 'Year of Birth: ' AS 'BIRTH', 2007 - Sage AS 'BIRTHDAY', LOWER(Sdept) AS 'DEPARTMENT'
FROM Student;
...
-- 查询所有选课记录中的学号（包括重复值）
SELECT ALL Sno AS '学号' FROM SC;

-- 查询计算机科学系（CS）的所有学生姓名
SELECT Sname AS '姓名' FROM Student WHERE Sdept = 'CS';

-- 查询所有年龄小于 20 岁的学生姓名和年龄
SELECT Sname AS '姓名', Sage AS '年龄' FROM Student WHERE Sage < 20;

-- 查询所有有不及格成绩（分数低于 60）的不同学生学号
SELECT DISTINCT Sno AS '学号' FROM SC WHERE Grade < 60;

-- 查询所有年龄在 20 到 30 岁之间（含）的学生姓名、系别和年龄
SELECT Sname AS '姓名', Sdept AS '系别', Sage AS '年龄'
FROM Student
WHERE
...
SELECT Sname AS '姓名', Sno AS '学号', Ssex AS '性别'
FROM Student
WHERE
    Sname LIKE 'M%';

-- 查询所有姓名为 'Joh' 开头且长度为 5 个字符的学生姓名
SELECT Sname AS '姓名' FROM Student WHERE Sname LIKE 'Joh__';

-- 查询所有姓名中第三个字符为 'r' 的学生姓名和学号
SELECT Sname AS '姓名', Sno AS '学号'
FROM Student
WHERE
    Sname LIKE '__r%';

-- 查询课程名精确为 'DB_Design' 的课程编号和学分
SELECT Cno AS '课程编号', Ccredit AS '学分'
FROM Course
WHERE
    Cname = 'DB_Design';
...
SELECT Sno AS '学号', Cno AS '课程编号' FROM SC WHERE Grade IS NULL;
...
-- 计算课程 '1' 的平均成绩
SELECT AVG(Grade) AS '课程 1 平均成绩' FROM SC WHERE Cno = '1';
...
-- 统计每门课程的选课人数
SELECT Cno AS '课程编号', COUNT(Sno) AS '选课人数' FROM SC GROUP BY Cno;

-- 查询选修了多于一门课程的学生学号
SELECT Sno AS '学号' FROM SC GROUP BY Sno HAVING COUNT(Cno) > 1;

-- 查询所有选课学生的学号、姓名、性别、系别、课程编号和成绩
SELECT S.Sno AS '学号', S.Sname AS '姓名', S.Ssex AS '性别', S.Sdept AS '系别', SC.Cno AS '课程编号', SC.Grade AS '成绩'
FROM Student AS S
    RIGHT JOIN SC ON S.Sno = SC.Sno;

-- 查询所有课程的先修课程关系，即每门课程及其直接先修课程的编号
SELECT C.Cno AS '课程编号', CP.Cno AS '先修课程编号'
FROM Course C
...
SELECT S.Sno AS '学号', S.Sname AS '姓名', C.Cname AS '课程名称', SC.Grade
FROM SC
