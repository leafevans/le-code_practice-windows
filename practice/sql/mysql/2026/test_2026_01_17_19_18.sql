...
SELECT e1.name AS `职员名字`, e2.name AS `上司名字`
FROM emp e1
    INNER JOIN emp e2 ON e1.managerid = e2.id;
...
SELECT e1.name AS `员工A`, e2.name AS `员工B`
FROM emp e1
...
SELECT e.name, e.age, e.job, e.dept_id, d.name
FROM emp e
    LEFT JOIN dept d ON e.dept_id = d.id;
