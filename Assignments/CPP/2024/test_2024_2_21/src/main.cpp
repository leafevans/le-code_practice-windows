...
class List {
 public:
  ...
  class Iterator {
   public:
    ...
    bool operator==(const Iterator &other) const {
      return m_start == other.m_start && m_cur == other.m_cur &&
             m_end == other.m_end;
    }
  ...
  class ConstantReverseIterator {
   public:
    ConstantReverseIterator(Node *start, Node *cur, Node *end)
        : m_rit(start, cur, end) {}

    const T &operator*() { return *m_rit; }
  ...
  Iterator begin() { return Iterator(m_head, m_head, m_tail); }

  Iterator end() { return Iterator(m_head, nullptr, m_tail); }
  ...
  ReverseIterator rbegin() { return ReverseIterator(m_tail, m_tail, m_head); }

  ReverseIterator rend() { return ReverseIterator(m_tail, nullptr, m_head); }
...
int main() {
  int arr[10] = {21, 332, 34, 22, 24, 56, 66, 89, 10, 100};
  for (int i = 0; i < 10; ++i) {
    std::cout << arr[i] << ' ';
  }
  std::cout << '\n';
