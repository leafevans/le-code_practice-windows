#ifndef INC_GRAPH_BASE_H_
#define INC_GRAPH_BASE_H_
#include <cstddef>
#include <iostream>
  ...
  while (!lsTopoStack.IsEmpty()) {
    ...
    for (int nAdjNode = FirstAdjNode(nNode, tWeight); nAdjNode >= 0;
      ...
      if (--arrInDegree[nAdjNode] == 0)
        lsTopoStack.Push(nAdjNode);  // 如果入度为 0，入栈
    }
  }
...
bool GraphBase<ElemType, NodeType, WeightType>::CriticalPath(
  ...
  for (int nNode = 0; nNode < m_nNodeCount; ++nNode) {
    for (int nAdjNode = FirstAdjNode(nNode, nAdjNode, tWeight); nAdjNode >= 0;
         nAdjNode = NextAdjNode(nNode, nAdjNode, tWeight)) {
      // 计算活动的最早和最迟开始时间
...
bool GraphBase<ElemType, NodeType, WeightType>::Dijkstra(int nStartNode,
                                                         ...
                                                         int* arrPreNode) {
  // 检查参数有效性
  if (nStartNode < 0 || nStartNode >= m_nNodeCount || !arrPathLen ||
      !arrPreNode || m_eType != DIR_NETWORK)
    return false;  // 参数无效

  ClearTag();  // 重置访问标记

  std::fill_n(arrPathLen, m_nNodeCount, InfniteWeight());
  std::fill_n(arrPreNode, m_nNodeCount, -1);

  // 初始化起始节点的路径长度和前驱节点
  arrPathLen[nStartNode] = 0;
  arrPathLen[nStartNode] = nStartNode;

  WeightType tMinWeight = arrPathLen[nStartNode];
  int nNode = nStartNode;
  ...
  for (int i = 1; i < m_nNodeCount; ++i) {
    ...
    for (int nAdjNode = FirstAdjNode(nNode, tWeight); nAdjNode >= 0;
      ...
      // 如果找到更短路径，则更新
      if (!GetTag(nAdjNode) &&
          arrPathLen[nAdjNode] > arrPathLen[nNode] + tWeight) {
        arrPathLen[nAdjNode] = arrPathLen[nNode] + tWeight;
        arrPreNode[nAdjNode] = nNode;
      }
    }

    // 从未访问的节点中选择路径最短的节点
    tMinWeight = InfniteWeight();
    for (int j = 0; j < m_nNodeCount; ++j) {
      if (!GetTag(j) && arrPathLen[j] < tMinWeight) {
        nNode = j;  // 更新节点
        tMinWeight = arrPathLen[j];
      }
    }
  }
