#ifndef INC_CHILD_PARENT_TREE_H_
#define INC_CHILD_PARENT_TREE_H_
#include <float.h>
...
class ChildParentTree : public TreeBase<ElemType, int>,
 ...
 public:
  ...
  // 构造函数, 通过数组构造树
  ChildParentTree(ElemType* arrElem, int* arrParents, int nRoot, int nDataLen,
                  int nBufferLen);
  // 赋值运算符重载
...
void ChildParentTree<ElemType>::ClearRemovedNode() {
  ...
  // 第一遍扫描: 建立新旧位置映射关系
  for (i = 0, j = 0; i < this->m_nDataLen; ++i) {
...
// 通过数组构造树实现
template <class ElemType>
ChildParentTree<ElemType>::ChildParentTree(ElemType* arrElem, int* arrParents,
  ...
  for (int i = 0; i < nBufferLen; ++i) {
    this->m_pData[i].m_tElem = arrElem[i];
...
#endif  // INC_CHILD_PARENT_TREE_H_