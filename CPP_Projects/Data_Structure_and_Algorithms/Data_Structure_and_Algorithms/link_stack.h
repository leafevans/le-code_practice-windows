...
class LinkStack : private SimpleLinkList<ElemType> {
 public:
  LinkStack();                                  // 默认构造函数
...
template <class ElemType>
LinkStack<ElemType>::LinkStack() : SimpleLinkList<ElemType>() {}

template <class ElemType>
LinkStack<ElemType>::LinkStack(const LinkStack<ElemType>& lsSrc)
    : SimpleLinkList<ElemType>(lsSrc) {}
...
void LinkStack<ElemType>::Clear() {
  SimpleLinkList<ElemType>::Clear();  // 调用基类函数
}

template <typename ElemType>
void LinkStack<ElemType>::Traverse(
    bool (*pVisit)(const ElemType& tElem)) const {
  SimpleLinkList<ElemType> Traverse(tElem);  // 调用基类函数
}
