...
class SqList : public ListBase<ElemType> {
 ...
 protected:
  ...
  bool Resize(int nSize);  // 内部调整缓冲区大小私有函数
...
template <class ElemType>
bool SqList<ElemType>::Resize(int nSize) {
  if (nSize <= m_nBufferLen) {  // 若新大小比原来的小
    return true;
  }
...
int SqList<ElemType>::SaveData(ElemType* pData, int nDataLen) {
  ...
  std::copy(pData, pData + nDataLen, m_pData + m_nDataLen);  // 拷贝数据
...
bool SqList<ElemType>::Insert(int nIdx, const ElemType& tElem) {
  ...
  if (m_nDataLen >= m_nBufferLen) {
    int nNewBufLen = m_nDataLen;  // 更新數據長度
  ...
  if (nIdx <= m_nDataLen - 1) {
    ...
    nIdx = m_nDataLen;  // 插入到数据的末尾
  }

  m_pData[nIdx] = tElem;  // 插入数据
  ++m_nDataLen;           // 更新数据长度
