...
namespace AnimalWellLight.Screens;
...
class MainMenuScreen : MenuScreen
{
    ...
    private void SetLanguageText()
    {
        ...
        Title = "AnimalWellLight"; // TODO uncomment this if you want it to use Resources.MainMenu; instead
    }
    ...
    public override void LoadContent()
    {
        ...
        settingsManager ??= ScreenManager.Game.Services.GetService<SettingsManager<AnimalWellLightSettings>>();
    ...
    private void UpdateTutorialSteps(GameTime gameTime)
    {
        if (gameTime.TotalGameTime - timeSinceLastMessage > TimeSpan.FromSeconds(3)) // Should the showtime be in settings?
        {
            ...
            if (AnimalWellLightGame.IsMobile)
            {
                if (tutorialStep > 3)
                {
                    tutorialStep = -1;
