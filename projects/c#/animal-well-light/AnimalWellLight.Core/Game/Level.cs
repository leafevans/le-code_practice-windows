...
using AnimalWellLight.Core.Inputs;
...
// Lighting system for Animal Well-style effects
using LightManager = AnimalWellLight.Core.Effects.LightingManager;
...
namespace AnimalWellLight.Core
{
    ...
    class Level : IDisposable
    {
        ...
        private SettingsManager<AnimalWellLightLeaderboard> settingsManager;
        private bool saved;
        ...
        private List<PointLight> gemLights = new List<PointLight>();
        ...
        /// </summary>
        public SettingsManager<AnimalWellLightLeaderboard> LeaderboardManager
        {
            get => settingsManager;
        ...
        public Level(ScreenManager screenManager, string levelPath, int levelIndex)
        {
            ...
            // Initialize Animal Well-style lighting system
            InitializeLighting();
        }
        ...
        /// <summary>
        /// Initializes the Animal Well-style lighting system with darkness, point lights, and atmospheric effects.
        /// Falls back to standard rendering if the lighting system fails to initialize.
        /// </summary>
        private void InitializeLighting()
        {
            try
            {
                ...
                lightingManager = new LightManager(
                ...
                // Configure for Animal Well-style darkness (playable brightness)
                lightingManager.AmbientDarkness = 0.5f;
                lightingManager.LightIntensityMultiplier = 1.5f;
                lightingManager.FogColor = new Color(20, 15, 40); // Dark purple tint
                lightingManager.VignetteStrength = 0.8f;

                // Add player light (warm glow, large radius)
                playerLight = lightingManager.AddLight(
                    Player.Position,
                    new Color(255, 230, 200), // Warm white
                    300f, // Large radius so you can see the area
                    1.2f  // Bright
                );

                // Add lights for each gem (blue/cyan glow like Animal Well)
                foreach (var gem in gems)
                {
                    var gemLight = lightingManager.AddLight(
                        gem.Position,
                        new Color(120, 200, 255), // Cyan/blue
                        140f, // Visible from distance
                        0.9f
                    );
                    gemLights.Add(gemLight);
                }

                // Add exit light (green glow)
                if (exit != InvalidPosition)
                {
                    lightingManager.AddLight(
                        new Vector2(exit.X, exit.Y),
                        new Color(120, 255, 160), // Green
                        160f,
                        1.0f
                    );
                }
            ...
            {
                // Lighting failed to initialize, fall back to standard rendering
                System.Diagnostics.Debug.WriteLine($"Lighting initialization failed: {ex.Message}");
                lightingManager?.Dispose();
                lightingManager = null;
            }

            // Add some atmospheric background lights (subtle, ambient)
            for (int i = 0; i < 5; i++)
            {
                float x = (Width * Tile.Width / 5) * i + (Width * Tile.Width / 10);
                float y = Height * Tile.Height * 0.3f;

                lightingManager.AddLight(
                    new Vector2(x, y),
                    new Color(40, 30, 60), // Very subtle purple
                    250f,
                    0.3f
                );
            }
        ...
        /// <summary>
        /// Unloads the level content and disposes of the lighting system.
        /// </summary>
        public void Dispose()
        {
            lightingManager?.Dispose();
        ...
        /// <summary>
        /// Updates the Animal Well-style lighting system, including light positions and pulsing effects.
        /// </summary>
        /// <param name="gameTime">Provides a snapshot of timing values.</param>
        private void UpdateLighting(GameTime gameTime)
        {
            ...
            lightPulseTime += (float)gameTime.ElapsedGameTime.TotalSeconds;
            ...
            if (playerLight != null)
            {
                ...
                // Subtle pulsing effect for the player light
                float pulse = 1.0f + MathF.Sin(lightPulseTime * 2.0f) * 0.1f;
                playerLight.Intensity = pulse;
                playerLight.Radius = 180f + MathF.Sin(lightPulseTime * 1.5f) * 10f;
            }

            // Update gem lights (remove lights for collected gems)
            for (int i = gemLights.Count - 1; i >= 0; i--)
            {
                if (i < gems.Count)
                {
                    // Update position for remaining gems
                    gemLights[i].Position = gems[i].Position;

                    // Gentle floating pulse for gem lights
                    float gemPulse = 1.0f + MathF.Sin(lightPulseTime * 3.0f + i * 0.5f) * 0.15f;
                    gemLights[i].Intensity = gemPulse * 0.8f;
                }
                else
                {
                    // Gem was collected, deactivate its light
                    gemLights[i].IsActive = false;
                }
            }

            // Adjust ambient darkness based on player state
            if (Player.IsPoweredUp)
            {
                // Brighter when powered up
                lightingManager.AmbientDarkness = 0.35f;
                playerLight.Color = new Color(255, 255, 220); // Bright white
                playerLight.Radius = 380f;
            }
            else
            {
                lightingManager.AmbientDarkness = 0.5f;
                playerLight.Color = new Color(255, 230, 200); // Warm white
                playerLight.Radius = 300f;
            }
        }
        ...
        /// <summary>
        /// Draws everything in the level with Animal Well-style lighting effects.
        /// Uses multi-pass rendering: scene pass → light map pass → final composition.
        /// </summary>
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            if (lightingManager != null)
            {
                ...
                lightingManager.EndScene();

                // ============================================
                // PASS 2: Render light map
                // ============================================
                lightingManager.RenderLightMap(new Vector2(cameraPosition, 0), transformScale);

                // ============================================
                // PASS 3: Apply lighting and render final output
                // ============================================
                lightingManager.ApplyLighting(gameTime);

                // Draw HUD on top of lighting (unaffected by darkness)
                spriteBatch.Begin(SpriteSortMode.Deferred, null, null, null, null, null, screenManager.GlobalTransformation);
            ...
            {
                // Fallback: Standard rendering without lighting
                // Draw background layers (layers behind entities).
                for (int i = 0; i <= EntityLayer; ++i)
                {
                    spriteBatch.Begin(SpriteSortMode.Deferred, null, null, null, null, null, screenManager.GlobalTransformation);
