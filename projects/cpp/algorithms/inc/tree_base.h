#ifndef INC_TREE_BASE_H_
#define INC_TREE_BASE_H_
#include <algorithm>
#include <iostream>
...
void TreeBase<ElemType, NodeHandle>::DeleteAux(NodeHandle hRoot) {
  if (NodeIsEmpty(hRoot))
    return;
  NodeHandle hChild = static_cast<NodeHandle>(GetFirstChild(hRoot));
  while (!NodeIsEmpty(hChild)) {
    NodeHandle hNextChild = static_cast<NodeHandle>(GetRightSibling(hChild));
