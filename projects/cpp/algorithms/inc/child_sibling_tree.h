#ifndef INC_CHILD_SIBLING_TREE_H_
#define INC_CHILD_SIBLING_TREE_H_
#include <cstddef>
...
class ChildSiblingTree
 ...
 private:
  ...
  // nRoot: 当前子树根结点的下标
  // nDataLen: 数组长度
  ChildSiblingTreeNode<ElemType>* CreateTreeAux(ElemType* arrElem,
...
template <class ElemType>
ChildSiblingTree<ElemType>::ChildSiblingTree(ElemType* arrElem, int* arrParents,
                                             int nRoot, int nDataLen)
    : m_pRoot(arrElem, arrParents, nRoot, nDataLen) {}
...
ChildSiblingTreeNode<ElemType>* ChildSiblingTree<ElemType>::CreateTreeAux(
    ElemType* arrElem, int* arrParents, int nRoot, int nDataLen) {
  if (nRoot < 0 || nRoot >= nDataLen)
    return NULL;

  // 创建根结点
  ChildSiblingTreeNode<ElemType>* pNewRoot =
      new ChildSiblingTree<ElemType>(arrElem[nRoot]);
  ChildSiblingTreeNode<ElemType>*pSubRoot, *pCurrNode;

  // 为当前结点创建子树
  for (int i; i < nDataLen; ++i) {
    if (arrParents[i] == nRoot) {
      pSubRoot = CreateTreeGAux(arrElem, arrParents, i, nDataLen);
      if (!(pNewRoot->m_pFirstChild)) {
        pNewRoot->m_pFirstChild = pSubRoot;  // 作为第一个孩子
      } else {
        pCurrNode->m_pRightSibling = pSubRoot;  // 加入兄弟链表
...
#endif  // INC_CHILD_SIBLING_TREE_H_