...
// AnswerReportDao 面试报告的答题记录表数据访问对象
var AnswerReportDao _AnswerReport

// AnswerReport 面试报告的答题记录表
type (
...
// GetAnswerReportByID 根据ID获取答题记录
func (dao *_AnswerReport) GetAnswerReportByID(id uint64) (*AnswerReport, error) {
	if getDB() == nil {
		panic("DB connection is not initialized")
	}
	var report *AnswerReport
	getDB().Where("id = ? AND deleted = 0", id).First(&report)
	return report, nil
}

// GetAnswerReportByUserIDAndReportID 根据用户ID和报告ID获取答题记录
func (dao *_AnswerReport) GetAnswerReportByUserIDAndReportID(userID uint, reportID uint64) (*AnswerReport, error) {
	if getDB() == nil {
		panic("DB connection is not initialized")
	}
	var report *AnswerReport
	getDB().Where("user_id = ? AND report_id = ? AND deleted = 0", userID, reportID).First(&report)
	return report, nil
}

// DeleteAnswerReport 软删除答题记录
...
// CountByReportID 根据报告ID统计答题记录数量
func (dao *_AnswerReport) CountByReportID(reportID uint64) (int64, error) {
	if getDB() == nil {
		panic("DB connection is not initialized")
	}
	var count int64
	err := getDB().Model(&AnswerReport{}).Where("report_id = ? AND deleted = 0", reportID).Count(&count).Error
	return count, err
}

// ListByUserIDAndReportID 根据用户ID和报告ID列出答题记录
func (dao *_AnswerReport) ListByUserIDAndReportID(userID uint, reportID uint64) ([]*AnswerReport, error) {
	if getDB() == nil {
		panic("DB connection is not initialized")
	}
	var reports []*AnswerReport
	err := getDB().Where("user_id = ? AND report_id = ? AND deleted = 0", userID, reportID).Find(&reports).Error
	return reports, err
}
