...
class LinkListBase : public ListBase<T> {
 public:
  ...
  virtual LinkListBase &operator=(const LinkListBase<T, NodeType> &src);  // 重载赋值操作符
...
template<class T, class NodeType>
LinkListBase<T, NodeType> &LinkListBase<T, NodeType>
::operator=(const LinkListBase<T, NodeType> &src) {
  clear();  // 清空原有链表
...
bool LinkListBase<T, NodeType>::delCurr(T &elem) {
  if (!curr_ || curr_ == head_) {  // 如果该节点为空或当前节点为头节点（循环链表）
    return false;
  }
